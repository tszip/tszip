...
import { resolveApp, safePackageName, clearConsole, getNodeEngineRequirement, } from './utils';
...
export const isDir = (name) => stat(name)
    .then((stats) => stats.isDirectory())
    .catch(() => false);
export const isFile = (name) => stat(name)
...
`));
    ...
    async function getProjectPath(projectPath) {
        ...
        const prompt = new Input({
            message: `A folder named ${chalk.bold.red(pkg)} already exists! ${chalk.bold('Choose a different name')}`,
            initial: pkg + '-1',
            result: (v) => v.trim(),
        });
    ...
    try {
        ...
        bootSpinner.start();
        // copy the template
        await fs.copy(path.resolve(__dirname, `../templates/${template}`), projectPath, {
            overwrite: true,
        });
        // fix gitignore
        await fs.move(path.resolve(projectPath, './gitignore'), path.resolve(projectPath, './.gitignore'));
        // update license year and author
        let license = await fs.readFile(path.resolve(projectPath, 'LICENSE'), { encoding: 'utf-8' });
...
prog
    ...
    watch(buildConfigs.map((inputOptions) => ({
        watch: {
            silent: true,
            include: ['src/**'],
...
async function cleanOldJS() {
    ...
    // console.log({ oldJS });
    await progressIndicator(Promise.all(oldJS.map(async (file) => await fs.unlink(file))), 'Removing original emitted TypeScript output (dist/**/*.js).');
}
...
prog
    ...
    let jestConfig = {
        ...createJestConfig((relativePath) => path.resolve(__dirname, '..', relativePath), opts.config ? path.dirname(opts.config) : paths.appRoot),
        ...appPackageJson.jest,
    };
